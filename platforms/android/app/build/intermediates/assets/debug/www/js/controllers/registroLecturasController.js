...
angular.module('app.registroLecturasController', [])

.controller('registroLecturasCtrl',function($scope,$cordovaGeolocation,$ionicPopup,$timeout,$state,sqliteServices,plantillaA1Services,loginServices,popupServices,$ionicActionSheet,$q,$ionicModal,ServicesPhoto,gpsServices,envioPendientesServices,$firebaseArray,$firebaseObject,$rootScope, $cordovaNetwork){
	...
	var DistanciaKM = 10;
	// WATCH MAP
	var soundMedia = function(){
		if ($scope.media) {
			$scope.media.pause();
		};
		$scope.media = new Audio();
		$scope.media.src = "/sound/alerta.mp3";
		$scope.media.load();
		$scope.media.play();
	}
	$timeout(function(){
		// CARGA DE SUMINISTRO PARA VALIDAR DISTANCIA
		/*$scope.paramsPosi = {
				lat : '0',
				long : '0',
				cant : '0',
			 ...
			 var watch = $cordovaGeolocation.watchPosition(watchOptions);
			  watch.then(
			    ...
			    function(position) {
			    	...
			    	$scope.paramsPosi.cant = 0;
					$scope.paramsPosi.lat = position.coords.latitude;
					$scope.paramsPosi.long = position.coords.longitude;
				
			        
			        var PosiActual = new google.maps.LatLng($scope.paramsPosi.lat,$scope.paramsPosi.long);
			        var latlng = {lat: parseFloat($scope.paramsPosi.lat), lng: parseFloat($scope.paramsPosi.long)};
			        $scope.paramsPosi.dire = "";
			      
					sqliteServices.getTbl_Suministros_All(dataUser.ID_Operario).then(function(res){	
						sqliteServices.getUltimoRegistro(dataUser.ID_Operario).then(function(resReg){		
							//alert(JSON.stringify(resReg))							
							if (resReg.length == 0) {
								return;
							}

							if (resReg[0].Registro_Latitud == 0 || resReg.length == 0) {
								return;
							}
							var ultimoLat = resReg[0].Registro_Latitud;
							var ultimoLon = resReg[0].Registro_Longitud;
							var posUltimaLec = new google.maps.LatLng(ultimoLat,ultimoLon);							
					        res.forEach(function(item,index){				        	
								var ubicLectura = new google.maps.LatLng(item.Suministro_Latitud,item.Suministro_Longitud);								
								var distancia = google.maps.geometry.spherical.computeDistanceBetween(posUltimaLec,ubicLectura);// EN METROS							
								//alert(distancia)
								if (distancia <= DistanciaKM) {								
									$scope.paramsPosi.cant ++;									
								};								
					        });
					        if ($scope.paramsPosi.cant > 0) {					        	
						    	//$scope.paramsPosi.lat = position.coords.latitude;
						        //$scope.paramsPosi.long = position.coords.longitude;
						        //PosiActual = new google.maps.LatLng($scope.paramsPosi.lat,$scope.paramsPosi.long);
					        	var distanciaNew = google.maps.geometry.spherical.computeDistanceBetween(PosiActual,posUltimaLec);
					        	//alert('Distancia entre el ultimo punto de registro y posi actual:' + distanciaNew)
					        	if (distanciaNew > DistanciaKM) {					        		
					        		//soundMedia();								
	...
	var tomaFoto = 0;
	var primerIngreso = 0;
	var tipoProcesoAux = '1,3';
	var showPopInferior = true;
	...
	$timeout(function(){
		...
		$scope.showDns = true;
		$scope.showSum = false;
		var itemFirst = localStorage.getItem('dataSuministro');				
		...
		if (itemFirst == 'undefined' || itemFirst == null) {
			...
			sqliteServices.getTbl_Suministros(0,'',dataUser.ID_Operario,tipoProcesoAux).then(function(data){				
				...
				localStorage.setItem('dataSuministro',JSON.stringify(data[0]));				
				executeInit();
			});
		}else{			
			if (!existeAux) {				
			...
			if (!existeAux2) { // si es falso
				...
				txtHeader = $scope.itemSuministro.SuministroOrden;				
				if (!existeAux) {					
					localStorage.setItem('dataSumAnterior',itemFirst)
				}

			}else{				
				$scope.itemSuministro = JSON.parse(localStorage.getItem('dataSumAnterior'));
				txtHeader = $scope.itemSuministro.SuministroOrden;				
				existeAux2 = false;				
			};
			 ...
			 /*var watch = $cordovaGeolocation.watchPosition(watchOptions);
			  watch.then(
			    ...
			    function(position) {
			        ...
			        res.forEach(function(item,index){									        	
						var ubicLectura = new google.maps.LatLng(item.Suministro_Latitud,item.Suministro_Longitud);
						var distancia = google.maps.geometry.spherical.computeDistanceBetween(PosiActual,ubicLectura);// EN METROS						
						if (distancia <= DistanciaKM) {
							$scope.paramsPosi.cant ++;
						};
...
// ULTIMA VALIDACIÓN , SI CONSUMOACTUAL > CONSUMOPROMEDIO X 2 , GUARDAR 2 ULTIMOS DIGITOS DEL MEDIDOR												
			...
			console.log(consumoActual,consumoPromedioAux);
				...
				$timeout(function(){
					...
					$scope.valicacionLecturas().then(function(res){		
							...
							if (res) {
								...
								//$scope.itemSuministro.ConsumoPromedio = 0
								...
								console.log('real',tomaFoto);			
									// VALIDAMOS SI LA LEECTURA ES VACIO
								if ($scope.paramsLecturasA.Registro_Lectura.length == 0) {
									tomaFoto = 1;									
								}
							if($scope.itemSuministro.ConsumoPromedio == 0 | $scope.itemSuministro.ConsumoPromedio == "0"){
								tomaFoto = 1;
							}
								var lecturaActual = String($scope.paramsLecturasA.Registro_Lectura) + '.' + String($scope.paramsLecturasA.Registro_LecturaD);
									var lecturaAnterior = $scope.itemSuministro.LecturaAnterior;
									var consumoActual = parseFloat(lecturaActual) - parseFloat(lecturaAnterior);	
									if ($scope.itemSuministro.ConsumoAnterior == "null") {
										$scope.itemSuministro.ConsumoAnterior = 0;
									};									

									if ($scope.itemSuministro.ConsumoAnterior == 0 && consumoActual == 0) {
										tomaFoto = 0;
									}
								
										
								// VALIDACIÓN DE OBSERVACIÓN PAR AFOTO
								if (tomaFotoObs == 1) {
									tomaFoto = 1;
								}
	...
	var validateRango = function(){
		...
		console.log($scope.itemSuministro);
		var lecturaD = 0;
				console.log($scope.itemSuministro.PorceCalculo);
					if ($scope.itemSuministro.PorceCalculo == 0) {
	...
	var nextSuministro = function(){
		...
		sqliteServices.getTbl_SuministrosById(idReg,ordenAuxM,ordenAux,tipoProcesoAux).then(function(res){			
			...
			q.resolve('success');
			localStorage.setItem('dataSuministro',JSON.stringify(res[0]));
		})
		return q.promise;
	};
