...
angular.module('app.registroVerificacionesController', [])

.controller('registroVerificacionesCtrl',function($scope,$ionicPopup,$timeout,$state,sqliteServices,plantillaA1Services,popupServices,$ionicActionSheet,$q,$ionicModal,ServicesPhoto,gpsServices,envioPendientesServices){
	// INIT CARGA DE LOAD	
	...
	$timeout(function(){
		...
		$scope.showDns = true;
		$scope.showSum = false;
		var itemFirst = localStorage.getItem('dataSuministro2');				
		...
		if (itemFirst == 'undefined' || itemFirst == null) {
			...
			sqliteServices.getTbl_Suministros(0,'',dataUser.ID_Operario,tipoProcesoAux).then(function(data){				
				...
				localStorage.setItem('dataSuministro2',JSON.stringify(data[0]));				
				executeInit();
			});
		}else{			
			if (!existeAux) {				
			...
			if (!existeAux2) { // si es falso
				...
				txtHeader = $scope.itemSuministro.SuministroOrden;				
				if (!existeAux) {					
					localStorage.setItem('dataSumAnterior2',itemFirst)
				}

			}else{				
				$scope.itemSuministro = JSON.parse(localStorage.getItem('dataSumAnterior2'));
				txtHeader = $scope.itemSuministro.SuministroOrden;				
				existeAux2 = false;				
			};
	...
	$scope.saveSelfie = function(){
		...
		console.log('SaveSelfie');
		//getPorceCalculo(2);
		var popAlert;
		var txtReturn;
		var inicio = 0;
		$timeout(function(){	
		if ($scope.itemSuministro.Suministro_LecturaMinima == "999999") {
			tomaFoto = 1;
			$scope.saveLectura();
		}else{
			tomaFotoNew = 0;
	...
	// ULTIMA VALIDACIÓN , SI CONSUMOACTUAL > CONSUMOPROMEDIO X 2 , GUARDAR 2 ULTIMOS DIGITOS DEL MEDIDOR												
				...
				tomaFoto = 0;
			//tomaFoto = validacionVerificacion();
	...
	var showModal = false;
	var validacionVerificacion = function(){
	...
	var insertLectura = function(params){
		...
		console.log($scope.paramsLecturas);
			...
			//					sqliteServices.updateEstatusTbl_Suministros(params)	
				...
				popupServices.alertPop('Error !' , "Ocurrio un error al registrar lectura 2 !");
				flagReLec = false;
				returnInsert = true;
				$scope.disabledInput = false;				
				//loadPop.close();
	...
	$scope.validateRange = function(){
		...
		if ($scope.itemSuministro.Suministro_LecturaMinima != "999999") {			
			...
			if(lecturaIngresadaEntero < lecturaAnteriorMenos1){
				...
				var pop = popupServices.alertPop('Alerta !' , "Fuera de rango.");
				pop.then(function(){
					var txtregLectura = document.getElementById('regLecturaD');
					$timeout(function(){
	...
	var nextSuministro = function(){
		...
		sqliteServices.getTbl_SuministrosById(idReg,ordenAuxM,ordenAux,tipoProcesoAux).then(function(res){			
			...
			q.resolve('success');
			localStorage.setItem('dataSuministro2',JSON.stringify(res[0]));
		})
		return q.promise;
	};
	...
	$scope.goToMap = function(){
		$state.go('menu.home-mapsLecturasUnit3');		
	}
	var tomaFotoObs = 0;
	...
	$scope.goToMap2 = function(){
		localStorage.setItem('idTipoProceso',2);
