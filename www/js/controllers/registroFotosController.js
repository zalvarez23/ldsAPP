...
angular.module('app.registroFotosController', [])

.controller('registroFotosCtrl',function($scope,$timeout,$state,sqliteServices,plantillaA1Services,popupServices,$ionicActionSheet,$q,$ionicModal,ServicesPhoto,gpsServices,envioPendientesServices){
	// INIT CARGA DE LOAD	
	var estado_general = "F";
	...
	$timeout(function(){
		...
		$scope.showDns = true;
		$scope.showSum = false;
		var itemFirst = localStorage.getItem('dataSuministro3');				
		...
		if (itemFirst == 'undefined' || itemFirst == null) {
			...
			sqliteServices.getTbl_Suministros_Foto(0,'',dataUser.ID_Operario,estado_general).then(function(data){				
				...
				localStorage.setItem('dataSuministro3',JSON.stringify(data[0]));				
				executeInit();
			});
		}else{			
			if (!existeAux) {				
			...
			if (!existeAux2) { // si es falso
				...
				txtHeader = $scope.itemSuministro.SuministroOrden;				
				if (!existeAux) {					
					localStorage.setItem('dataSumAnterior3',itemFirst)
				}

			}else{				
				$scope.itemSuministro = JSON.parse(localStorage.getItem('dataSumAnterior3'));
				txtHeader = $scope.itemSuministro.SuministroOrden;				
				existeAux2 = false;				
			};
...
//					sqliteServices.updateEstatusTbl_Suministros(params)	
	...
	var nextSuministro = function(){
		...
		sqliteServices.getTbl_SuministrosById_Foto(idReg,ordenAuxM,ordenAux,estado_general).then(function(res){			
			...
			q.resolve('success');
			localStorage.setItem('dataSuministro3',JSON.stringify(res[0]));
		})
		return q.promise;
	};
